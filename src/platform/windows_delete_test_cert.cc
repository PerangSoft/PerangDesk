...
//*************************************************************
//
//  DeletePerangDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes PerangDesk Test certificates and wrong key stores
//
...
BOOL DeletePerangDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeletePerangDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeletePerangDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeletePerangDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeletePerangDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
