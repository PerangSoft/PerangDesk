...
const PERANGDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.perangdesk.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(PERANGDESK_CLIPBOARD_OWNER_FORMAT),
];
...
pub fn is_file_url_set_by_perangdesk(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.perangdesk_", "//tmp/.perangdesk_"] {
...
fn do_update_clipboard_(mut to_update_data: Vec<ClipboardData>, side: ClipboardSide) {
    ...
    if let Some(ctx) = ctx.as_mut() {
        to_update_data.push(ClipboardData::Special((
            PERANGDESK_CLIPBOARD_OWNER_FORMAT.to_owned(),
            side.get_owner_data(),
        )));
...
impl ClipboardContext {
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == PERANGDESK_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != PERANGDESK_CLIPBOARD_OWNER_FORMAT,
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_perangdesk(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_perangdesk(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_perangdesk(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_perangdesk(data, side);
            if !urls.is_empty() {
                // FIXME:
                ...
                let clear_holder_text = if is_kde_x11 {
                    "PerangDesk placeholder to clear the file clipbard"
                } else {
                    ""
                }
                .to_string();
