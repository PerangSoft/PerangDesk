...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in PerangDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in PerangDesk
            ...
            // we consider only PerangDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/PerangDesk.app/Contents/MacOS/perangdesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run perangdesk from cmd
        std::thread::spawn(|| {
