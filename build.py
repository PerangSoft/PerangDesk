...
hbb_name = 'perangdesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: perangdesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: perangdesk <info@perangdesk.com>
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/perangdesk')
    ...
    system2('rm tmpdeb/usr/bin/perangdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/rustdesk/')
    system2(
        'cp ../res/perangdesk.service tmpdeb/usr/share/rustdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/perangdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/perangdesk.svg')
    system2(
        'cp ../res/perangdesk.desktop tmpdeb/usr/share/applications/perangdesk.desktop')
    system2(
        'cp ../res/perangdesk-link.desktop tmpdeb/usr/share/applications/perangdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/rustdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/rustdesk/')
    system2(
        'cp ../res/pam.d/perangdesk.debian tmpdeb/etc/pam.d/perangdesk')
    system2(
    ...
    system2('dpkg-deb -b tmpdeb perangdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('perangdesk.deb', '../perangdesk-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/PerangDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"PerangDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon PerangDesk.app 200 190 --hide-extension PerangDesk.app perangdesk.dmg ./build/macos/Build/Products/Release/PerangDesk.app")
    os.rename("perangdesk.dmg", f"../perangdesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/perangdesk.exe')
    os.chdir('../..')
    if os.path.exists('./perangdesk_portable.exe'):
        os.replace('./target/release/perangdesk-portable-packer.exe',
                   './perangdesk_portable.exe')
    else:
        os.rename('./target/release/perangdesk-portable-packer.exe',
                  './perangdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/perangdesk_portable.exe')
    os.rename('./perangdesk_portable.exe', f'./perangdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/perangdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/perangdesk.exe')
        system2('mv target/release/perangdesk.exe target/release/PerangDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\perangdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/PerangDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/perangdesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/perangdesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/perangdesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv perangdesk-%s-0-x86_64.pkg.tar.zst perangdesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./perangdesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/perangdesk-%s-0.x86_64.rpm ./perangdesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall perangdesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/perangdesk-%s-0.x86_64.rpm ./perangdesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall perangdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/perangdesk*.deb ./flutter/perangdesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/PerangDesk.app/Contents/MacOS/perangdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/PerangDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/perangdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/PerangDesk.app/Contents/MacOS/perangdesk
    #rcodesign sign --p12-file ~/.p12/perangdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/PerangDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/perangdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/PerangDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/PerangDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/PerangDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "PerangDesk %s.dmg" "target/release/bundle/osx/PerangDesk.app"' % version)
                os.rename('PerangDesk %s.dmg' %
                          version, 'perangdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/perangdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./perangdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./perangdesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple perangdesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/PerangDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/perangdesk*.deb ./perangdesk.deb')
                system2('dpkg-deb -R perangdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/rustdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                os.system('cp pam.d/perangdesk.debian tmpdeb/etc/pam.d/perangdesk')
                system2('strip tmpdeb/usr/bin/perangdesk')
                system2('mkdir -p tmpdeb/usr/share/perangdesk')
                system2('mv tmpdeb/usr/bin/perangdesk tmpdeb/usr/share/rustdesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/rustdesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb perangdesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('perangdesk.deb', 'perangdesk-%s.deb' % version)
