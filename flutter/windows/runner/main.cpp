...
typedef char** (*FUNC_PERANGDESK_CORE_MAIN)(int*);
typedef void (*FUNC_PERANGDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_PERANGDESK_GET_APP_NAME)(wchar_t*, int);
                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  FUNC_PERANGDESK_CORE_MAIN perangdesk_core_main =
      (FUNC_PERANGDESK_CORE_MAIN)GetProcAddress(hInstance, "perangdesk_core_main_args");
  if (!perangdesk_core_main)
  {
    std::cout << "Failed to get perangdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_PERANGDESK_FREE_ARGS free_c_args =
      (FUNC_PERANGDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = perangdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "PerangDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"PerangDesk";
  FUNC_PERANGDESK_GET_APP_NAME get_perangdesk_app_name = (FUNC_PERANGDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_perangdesk_app_name");
  if (get_perangdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_perangdesk_app_name(app_name_buffer, 512) == 0) {
